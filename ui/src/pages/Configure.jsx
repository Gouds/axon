...
export default function Configure() {
  ...
  // Save to disk + reload registry in one action
  async function apply() {
    setApplying(true)
    const res = await fetch(`/profiles/${activeProfile.id}`, {
      method: 'PUT',
    ...
    setApplying(false)
    flash(`Saved — registry reloaded.`)
  }

  if (!activeProfile) {
    return <div className="page"><p className="empty">No profile active. Go to Profiles to activate one.</p></div>
  }
  if (!profile) {
                ...
                >
                  ...
                  <div className="device-card-controls">
                    <button
                      className="btn-icon"
                      title="Remove"
                      onClick={(e) => { e.stopPropagation(); setPendingRemove(device.id) }}
                    >×</button>
